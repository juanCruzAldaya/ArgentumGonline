extends Node2D
## Wires the network client to the view and turns held keys into move commands.
...
var _tick := 0
var _visible_entities := 0


func _ready() -> void:
	randomize()
	_url = _resolve("server", "JUEGITO_SERVER", DEFAULT_URL)
	...
	_status.text = "conectando a %s ..." % _url
...
## Command line wins over environment, so two clients can be launched against
## different servers from the same shell.
func _resolve(flag: String, env_var: String, fallback: String) -> String:
	var prefix := "--%s=" % flag
...
func _on_connected() -> void:
	_connected = true
	_status.text = "conectado como %s — esperando welcome" % _player_name


func _on_disconnected() -> void:
	_connected = false
	_status.text = "desconectado de %s" % _url


func _on_welcomed(welcome: Dictionary) -> void:
	_view.configure(welcome)
	_status.text = "%s  |  id %d  |  mapa %dx%d  |  %d Hz" % [
		_player_name,
		int(welcome.get("id", 0)),
		int(welcome.get("w", 0)),
		int(welcome.get("h", 0)),
...
func _on_snapshot(snapshot: Dictionary) -> void:
	...
	_visible_entities = entities.size()
	_view.set_entities(entities)
	_status.text = "%s  |  tick %d  |  a la vista: %d  |  WASD / flechas" % [
		_player_name, _tick, _visible_entities
	]
